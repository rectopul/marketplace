...
module.exports = {
    ...
    async location(req, res) {
        ...
        const images = banner.map(async (item) => {
            ...
            try {
                ...
                if (error) console.log('Erro Baner: ', error)
            }
        })
    ...
    async delete(req, res) {
        ...
        try {
            ...
            if (!select) {
                ...
                await ImagesBanners.destroy({ where: { id: select.image_id } })
            }
            /** Remove Image */

            await Banners.destroy({ where: { id }, individualHooks: true })
    ...
    async update(req, res) {
        ...
        try {
            ...
            if (!select) return res.status(400).json({ error: 'The selected banner does not exist' })

            const banner = await Banners.update(
                { title, description, position, image_id },
    ...
    async store(req, res) {
        ...
        if (!authHeader) return res.status(401).send({ error: 'No token provided 2' })
        ...
        if (!store) return res.status(400).json({ error: 'User does not belong to informed store' })
